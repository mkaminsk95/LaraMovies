...
<form x-show="opened">
    <div class="flex flex-col fixed overflow-scroll inset-0 z-10 md:sticky md:top-12 md:mt-[52px] pt-8 md:pt-2 pb-6 px-10 h-full md:h-fit md:w-[320px] lg:w-[350px]
            bg-light-element dark:bg-dark-element shadow-lg dark:shadow-shadow-color border border-1 border-gray-300 dark:border-gray-700 rounded">
        ...
        <x-inputs.label class="mt-6" for="sorting">Sorting</x-inputs.label>
        ...
        <x-inputs.label class="mt-2" for="title">Title</x-inputs.label>
        <x-inputs.text id="title" name="title" :value="$query['title'] ?? ''"/>

        <x-inputs.label class="mt-2" for="title">Year</x-inputs.label>
        <x-inputs.select id="year" name="year" :options="range(date('Y'), 1900)" :value="$query['year'] ?? ''"/>

        <x-inputs.label class="mt-2" for="vote_average">Vote average</x-inputs.label>
        <x-inputs.select id="vote_average" name="vote_average" :options="range(1, 10)"
                        :value="$query['vote_average'] ?? ''"/>

        <x-inputs.label class="mt-4 mb-2">Genre</x-inputs.label>
        <div class="grid grid-cols-2 gap-2 text-sm">
            @foreach($genres as $genre)
                <div class="flex flex-row gap-2 items-center">
                    ...
                    <label class="text-sm" for="{{ $genre }}">{{ $genre }}</label>
                </div>
            @endforeach
        </div>
        <x-buttons.primary class="mt-8 mb-6">Search</x-buttons.primary>
    </div>
</form>
